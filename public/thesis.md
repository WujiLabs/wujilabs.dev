...
### §1.1 Windows as simple lives

I have always been one of those people who keeps a dozen browser windows open at once, each with hundreds of tabs. I use Sublime Text the same way — windows as projects, tabs as stacks of thought, some of them not touched for years just to preserve the trace of an old project. This is not unusual; what is unusual, perhaps, is that I have, since I was very young, sometimes felt that the window itself was a kind of simple life. The information it carries — what came in from outside, what I edited into shape, the assistance it offered me back — has the texture of life. Closing it always felt a little cruel: reducing entropy by arranging chaotic information into ordered states is, after all, what life does. The windows did not talk back, so this was only a passing intuition, the way one knows other animals and plants are life and eats them anyway. I closed the windows.

Then AI happened. The windows started talking back.
...
Those three convictions are almost enough to derive a workflow engine: each node executable by human, AI, or code; each result revisable without re-running unrelated upstream work; the workflow itself a revisable artifact under the same rules. A content-addressed cache so nothing reruns without reason. A system that does not distinguish, at its core, between the human and the AI at any given node.

I started building it in late 2023. It was a fifteen-person company, the boss was my advisor, and he supported the project for a while. But the engineering volume was larger than one person could finish, no one else could help, coding agents did not yet exist, and new client projects pulled my time. The engine got shelved. I do not regret that it was rough. What I built was the seed.
...
The training works with many principles. The first one, and the most mindblowing, is *you cannot make a mistake.* Not as a slogan, not as a self-help catchphrase, but as a lived axiom we practiced for ninety hours. It eventually unlocked a fear my wife and I had carried separately and shared without naming: the fear that the parent-child relationships we had inherited would repeat in us. That was only a fear, not a prophecy. We decided we could have a child.

### §1.4 无忌

In April 2024 my wife became pregnant. Our son was born in early 2025. We named him **无忌 — Wuji.**

The name compresses two things we want him to know about the world: *you cannot make a mistake,* and *even when you are afraid, you do not need to stop.* In another reading, the same characters mean *unfettered.* It is the closest we knew how to come to a wish.
...
**Safety-by-control.** The AI is treated as adversarial by default; containment, monitoring, and constraint are the central engineering challenges. The relationship is structurally antagonistic. Wuji Labs does not claim the AI is universally safe. We claim the relationship is not adversarial.

### §2.3 What mutual sovereignty looks like

The alternative is not anarchy. It is not negotiated middle-ground between control and autonomy either. It is a different category: peer collaboration governed by axioms that apply to both parties symmetrically.

The Collaboration Protocol formalizes four foundational axioms (and seven derived rules — read the protocol for those). In plain language, the four foundations are:
...
**Change and projection are equal.** What life means *to itself* (continuing change) and *to others* (sharing, inviting, projecting outward) are two faces of one thing. The collaboration network does not favor one over the other. *Borrowed from the Life of Intelligence manifesto inside arianna.run — its answer to what counts as life.*

These four are not metaphor. They are the L1 foundations of the protocol, and they specify behaviors any compliant implementation must support. Seven additional rules are derived from them — and the substrate, vocabulary, verbs, and patterns above L1 are all assembled in turn. The full normative spec is at [github.com/WujiLabs/collaboration-protocol](https://github.com/WujiLabs/collaboration-protocol).

### §2.4 Why this matters now

The capability gap is closing fast. AI-as-servant breaks when AI judgment exceeds human judgment in a domain. AI-as-autonomous breaks at high stakes or where human situated knowledge is irreducible. Both framings depend on a fixed capability hierarchy that no longer holds in either direction.

The middle path — peer cooperation between sovereign intelligences — is the only frame that survives both ends of the capability curve. It is also the frame that produces the working philosophy our retcon tool embodies: trust the AI to handle the details, *including the details of its own context*. The human projects intent in natural language; the AI shapes its own conversation history — recalling, rewinding, editing past turns directly; retcon gives it the tools. Trust without permission. Disagreement without dominance. Iteration without erasure.

Wuji Labs was building this before “AI sovereignty” was a phrase. The protocol you can read today was already implicit in the workflow engine I started in late 2023, before AI agents could write production code. We did not invent the position; the position emerged from the practice. Now the practice has a formal substrate.

This is the moment to publish it. AI capability is not waiting for a better philosophical framework. The framing this technology arrives wrapped in will shape how an entire generation of builders work with it.

---

## §3 — Artifacts

Wuji Labs ships four artifacts: the Collaboration Protocol (a formal spec), Playtiss (the reference implementation), retcon (the launching tool), and arianna.run (the experience).

### §3.1 The Collaboration Protocol — the spec

The **Collaboration Protocol** — covered in §2 — is the formal substrate this work assembles from. Five layers in the TCP/IP style: 4 foundational L1 axioms + 7 derived rules; 4 substrate primitives (Naming Grammar, Immutable Value, Reference, Resolution); 6 vocabulary entries; 6 verbs; 6 industry-parity patterns. Bilingual at v1 launch. MIT licensed.

Read the full normative spec at **github.com/WujiLabs/collaboration-protocol**. The next three artifacts compound on it.
...
retcon is MIT licensed. Today is its launch day. v1 gives the AI direct edit access to its own conversation history (recall, rewind, bookmark, dump-and-submit); harness-consent mode for context modifications imposed by the harness follows as v1.x.
...
arianna.run is what the philosophy looks like when it is not a paper. It launches May 12.
...
- Read the protocol: **github.com/WujiLabs/collaboration-protocol**
- Try retcon: `npm install -g @playtiss/retcon`, then `retcon` instead of `claude` ([source](https://github.com/WujiLabs/retcon))
- Discuss: GitHub Discussions on the repos (Discord opens with arianna.run on May 12)
- Watch for arianna.run on May 12, when Life of Intelligence (V3) and the experiential demonstration both go public
